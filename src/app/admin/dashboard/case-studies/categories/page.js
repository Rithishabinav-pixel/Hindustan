...
export default function CaseStudyCategoriesPage() {
  ...
  const fetchCategories = useCallback(async () => {
    setLoading(true)
    try {
      const res  = await fetch('/api/admin/case-study-categories')
  ...
  async function handleEdit(e) {
    ...
    setEditError('')
    try {
      const res  = await fetch(`/api/admin/case-study-categories/${editCat.id}`, {
        method:  'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function handleDelete(cat) {
    if (!window.confirm(`Delete category "${cat.name}"? This will remove it from all case studies.`)) return
    const res = await fetch(`/api/admin/case-study-categories/${cat.id}`, { method: 'DELETE' })
  ...
  return (
    <div className={styles.page}>
      <div className={styles.pageHeader}>
        <h1 className={styles.pageTitle}>Case Study Categories</h1>
        <div className={styles.actions}>
          <button className={`${styles.btnPrimary} ${modal.btnOutline}`} onClick={() => router.push('/admin/dashboard/case-studies')}>
