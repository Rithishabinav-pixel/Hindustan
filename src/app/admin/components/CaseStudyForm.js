...
export default function CaseStudyForm({ caseStudyId }) {
  ...
  const loadCategories = useCallback(async () => {
    ...
    try {
      const r    = await fetch('/api/admin/case-study-categories')
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/case-studies/${caseStudyId}`)
      ...
      .catch(() => setError('Failed to load case study data.'))
  ...
  function handleChange(e) {
    ...
    if (name === 'title' && !slugTouchedRef.current) {
      ...
      if (!canonicalTouchedRef.current) {
        setSeo((prev) => ({ ...prev, seoCanonicalUrl: generated ? `/case-study/${generated}` : '' }))
      }
      return
    }
    ...
    if (name === 'slug') {
      ...
      if (!canonicalTouchedRef.current) {
        setSeo((prev) => ({ ...prev, seoCanonicalUrl: value ? `/case-study/${value}` : '' }))
      }
    }
  ...
  return (
    <div className={styles.page}>
      <div className={styles.pageHeader}>
        <h1 className={styles.pageTitle}>{isEdit ? 'Edit Case Study' : 'Add New Case Study'}</h1>
      </div>
      ...
      <form className={styles.form} onSubmit={handleSubmit}>
        ...
        <div className={styles.section}>
          <h2 className={styles.sectionTitle}>Case Study Details</h2>

          <div className={styles.row}>
            <div className={styles.field}>
              <label className={styles.label}>Title *</label>
              <input className={styles.input} type="text" name="title" value={fields.title}
                onChange={handleChange} required disabled={loading} placeholder="Case study title" />
            </div>
            <div className={styles.field}>
              ...
              <input className={styles.input} type="text" name="slug" value={fields.slug}
                onChange={handleChange} required disabled={loading} placeholder="e.g. drone-agriculture-case-study" />
            </div>
          </div>
        ...
        <div className={styles.section}>
          ...
          {categories.length === 0 ? (
            <p style={{ fontSize: 13, color: '#9ca3af' }}>No categories yet. Add one from the Case Study Categories page, then click Refresh.</p>
          ) : (
        ...
        <div className={styles.section}>
          ...
          <div className={styles.field}>
            ...
            <TagInput value={tags} onChange={setTags} disabled={loading} tagsEndpoint="/api/admin/case-study-tags" />
          </div>
        </div>
        ...
        <div className={styles.section}>
          ...
          <div className={styles.field}>
            <label className={styles.label}>Case Study Content</label>
        ...
        <div className={styles.formActions}>
          <button type="button" className={styles.btnCancel}
            onClick={() => router.push('/admin/dashboard/case-studies')} disabled={loading}>
            Cancel
          </button>
          <button type="submit" className={styles.btnSave} disabled={loading}>
            {loading ? 'Saving…' : isEdit ? 'Save Changes' : 'Publish Case Study'}
          </button>
        </div>
      </form>
    </div>
